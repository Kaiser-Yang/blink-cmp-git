...
local function default_gitlab_issue_get_kind_name(_) return 'Issue' end
...
local function default_gitlab_mr_get_kind_name(_) return 'MR' end
...
local function default_gitlab_mr_or_issue_get_documentation(item, is_mr)
    return utils.concat_when_all_true(is_mr and '!' or '#', item.iid, ' ', item.title, '\n')
        .. utils.concat_when_all_true(
            'State: ',
            item.discussion_locked and 'locked' or item.draft and 'draft' or item.state,
            '\n'
        )
        ...
        .. (item.state == 'merged' and utils.concat_when_all_true(
            'Merged  at: ',
            item.merged_at,
            '\n'
        ) .. utils.concat_when_all_true('Merged  by: ', item.merged_by.username, '') .. utils.concat_when_all_true(
            ' (',
            item.merged_by.name,
            ')'
        ) .. '\n' or item.state == 'closed' and utils.concat_when_all_true(
            'Closed  at: ',
            item.closed_at,
            '\n'
...
local function default_gitlab_mention_get_kind_name(_) return 'Mention' end

local function default_gitlab_mention_get_insert_text(item)
    return utils.concat_when_all_true('@', item.username, '')
end

local function default_gitlab_get_command() return utils.command_found('glab') and 'glab' or 'curl' end
...
return {
    ...
    pull_request = {
        ...
        get_command = default_gitlab_get_command,
        get_command_args = function(command, token)
            return default_gitlab_mr_issue_mention_get_command_args(
                command,
                token,
                'merge_requests'
            )
